...
#  error "Unsupported platform"
    ...
    std::expected<void,err::Error> CryptoSodium::sodiumKeyGenerateBySalt(const std::vector<unsigned char> &salt)
    {
    ...
    std::expected<CryptoInfo,err::Error> CryptoSodium::encode(const std::string &plaintext)
    {
        if (salt_.empty())
        {
            std::vector<unsigned char> fresh_salt(crypto_pwhash_SALTBYTES);
            randombytes_buf(fresh_salt.data(), fresh_salt.size());
            if (const std::expected<void,err::Error> generating_result = sodiumKeyGenerateBySalt(fresh_salt); !generating_result.has_value())
            {
    ...
    std::expected<std::string,err::Error> CryptoSodium::decode(const CryptoInfo &crypto_info)
    {
